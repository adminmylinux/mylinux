...
Clipboard (2026-09-09 10:40): text clipboard Mac -> guest automatic, guest -> Mac on request (⌘⌃C / Ctrl+Alt+C, like
Omarchy). run.sh's host agent mirrors pbpaste into share/clipboard/mac.txt and pbcopies share/clipboard/guest.txt
(CLIPBOARD=0 disables); clipboard-bridge (S99shell) applies mac.txt with wl-copy; clipboard-send writes wl-paste output
to guest.txt. wl-clipboard needs a 1x1 popup surface for keyboard focus on a plain wl_data_device compositor (no
data-control protocol), which used to get tiled and flash: tiling now waits for the app id (finishAdd), and a mapped
surface without app id that is 1x1 or titled "wl-clipboard" becomes a helper: rendered 1x1 at 1% opacity (it must stay
